...
const MedicalExam = () => {
    ...
    const milkProductionUrl = 'http://localhost:5000/medicalExamination';
    ...
    const getProject = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/medicalExamination/`);
            const jsondata = response.data;
            console.log(jsondata);
            setProjects(jsondata.medExam);
            setLoading(false);
        } catch (err) {
            toast({
    ...
    return (
        <Flex h="100vh" justifyContent="center" alignItems="center">
            ...
            >
                ...
                <TaskTable  url={milkProductionUrl} />
            </Box>
        </Flex>
    );
};
